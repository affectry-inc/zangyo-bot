...
function ZangyoBot(controller) {
        ...
        if (isValidDate(date)) {
          findTimezone(response.team, response.user, function(err, timezone){
            if (err || !timezone) {
              ...
              if (!zangyo.is_delay && date < moment().tz(timezone).format('YYYY-MM-DD')) {
  ...
  function buildSimplifiedMessage(bot, message, range, applicant, filter) {
    findTargetZangyo(message, range, applicant, filter, function(zangyo){
      ...
      if (fields.length == 0) {
        reply = 'There is no ' + filter + ' items ' + applicant_text + 'of ' + range + '.';
      } else if (fields.length < 100) {
        reply = {
          'text': 'The ' + filter + ' list ' + applicant_text + 'of ' + range + '.',
          'attachments': [
            {
              'color': '#aaa',
              'fields': fields
        ...
        reply = {
          'text': 'The ' + filter + ' list ' + applicant_text + 'of ' + range + ' has more than 100 zangyos. Please adjust search conditions... :face_with_rolling_eyes:',
          'attachments': [
            {
              'color': '#aaa',
              'fields': fields
  ...
  function buildDetailedMessage(bot, message, range, applicant, filter) {
    findTargetZangyo(message, range, applicant, filter, function(zangyo){
      ...
      if (attachments.length == 0) {
        ...
        reply = {
          'text': 'The detailed ' + filter + ' list ' + applicant_text + 'of ' + range + '.',
