...
function ZangyoBot(controller) {
  ...
  zangyo_bot.approve = function(zangyo_id, bot, message) {
    getTeamZangyo(message.team.id, zangyo_id, function(team, idx) {
      ...
      controller.storage.teams.save(team);
      var zangyo = team.zangyos[idx];
      bot.replyInteractive(message, getSummary(zangyo, true, true, true));

      bot.configureIncomingWebhook(team.incoming_webhook);
      bot.sendWebhook(getSummary(zangyo, true, false, true), function(err,res) {
  ...
  zangyo_bot.rejectApprove = function(zangyo_id, bot, message) {
    getTeamZangyo(message.team.id, zangyo_id, function(team, idx) {
      ...
      controller.storage.teams.save(team);
      var zangyo = team.zangyos[idx];
      bot.replyInteractive(message, getSummary(zangyo, false, true, true));

      bot.configureIncomingWebhook(team.incoming_webhook);
      bot.sendWebhook(getSummary(zangyo, false, false, true), function(err,res) {
        if (err) console.log(err);
      });
    });
  };
  ...
  zangyo_bot.rejectCommentApprove = function(zangyo_id, bot, message) {
    getTeamZangyo(message.team.id, zangyo_id, function(team, idx) {
      ...
      var zangyo = team.zangyos[idx];
      bot.replyInteractive(message, getSummary(zangyo, false, true, false));
    });
  ...
  function getSummary(zangyo, is_approved, is_reply, is_done) {
    var result_word, color, text, fields;
