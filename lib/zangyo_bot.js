...
function ZangyoBot(controller) {
  ...
  function askApprover(convo) {
    convo.ask("プロマネは誰？ [@xxx]", function(response, convo) {
      if (response.text.match(/(cancel|キャンセル)/)) {
        convo.say('キャンセルしたよ！');
        convo.next();
  ...
  zangyo_bot.replyList = function(bot, message, range, is_detailed) {
    if (is_detailed) {
      buildDetailedMessage(bot, message, range);
    } else {
      buildSimplifiedMessage(bot, message, range);
    }
  };

  function buildSimplifiedMessage(bot, message, range) {
    findTargetZangyo(message.team, range, function(zangyo){
      var field = {};
      if (zangyo.approved == true) {
        field.value = ":white_check_mark: <@" + zangyo.applicant + "> - " + zangyo.end_time;
      } else {
        field.value = ":x: <@" + zangyo.applicant + "> - " + zangyo.end_time;
      }
      field.short = false
  ...
  function buildDetailedMessage(bot, message, range) {
    findTargetZangyo(message.team, range, function(zangyo){
      var attachment = {};
      if (zangyo.approved == true) {
      ...
      attachment.title = "<@" + zangyo.applicant + "> - " + zangyo.end_time;
      attachment.text = zangyo.reason;
      attachment.ts = moment().unix();
  ...
  function findTargetZangyo(team_id, range, getMessageItem, replyWholeMessage) {
    ...
    switch (range) {
      ...
      default:
        ...
        begin_date = moment().format("YYYY-") + target[0] + '-' + target[1];
        end_date = moment().format("YYYY-") + target[0] + '-' + target[1];
        break;
    }
    ...
    controller.storage.teams.get(team_id, function(err, team) {
      ...
      for (var i = 0; i < team.zangyos.length; i++) {
        ...
        if (begin_date <= zangyo.date && zangyo.date <= end_date) {
          list.push(getMessageItem(zangyo));
        }
      }

      replyWholeMessage(list);
    });
  };
