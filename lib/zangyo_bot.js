...
function ZangyoBot(controller) {
  ...
  function askReason(convo, zangyo) {
    convo.ask("残業する理由は？", function(response, convo) {
      ...
      var summary = getApplicationSummary(zangyo);
  ...
  function getApplicationSummary(zangyo) {
    var summary = {
      "text": "残業申請する？",
      "attachments": [
        {
  ...
  zangyo_bot.buildMessage = function(bot, message, range, applicant, filter, is_detailed) {
    if (is_detailed) {
      return buildDetailedMessage(bot, message, range, applicant, filter);
    } else {
      return buildSimplifiedMessage(bot, message, range, applicant, filter);
    }
  };
