...
function ZangyoBot(controller) {
  ...
  zangyo_bot.replyList = function(bot, message, range, applicant, is_detailed) {
    if (is_detailed) {
      buildDetailedMessage(bot, message, range, applicant);
    } else {
      buildSimplifiedMessage(bot, message, range, applicant);
    }
  };

  function buildSimplifiedMessage(bot, message, range, applicant) {
    findTargetZangyo(message.team, range, applicant, function(zangyo){
      var field = {};
      ...
      if (fields.length == 0) {
        reply = applicant_text + range + "の残業申請は１件もないっす。。。";
      } else {
        reply = {
          "text": applicant_text + range + "の残業申請一覧っす。",
          "attachments": [
            {
              "color": "#aaa",
              "fields": fields
  ...
  function buildDetailedMessage(bot, message, range, applicant) {
    findTargetZangyo(message.team, range, applicant, function(zangyo){
      var attachment = {};
      ...
      return attachment;
    }, function(attachments, applicant){
      var reply;
      var applicant_text = applicant ? "<@" + applicant + ">の" : "";
      if (attachments.length == 0) {
        reply = applicant_text + range + "は残業申請１件もないっす。。。";
      } else {
        reply = {
          "text": applicant_text + range + "の残業申請一覧を詳しくね。ほれっ！",
  ...
  function findTargetZangyo(team_id, range, applicant, getMessageItem, replyWholeMessage) {
    ...
    controller.storage.teams.get(team_id, function(err, team) {
      ...
      replyWholeMessage(list, applicant);
    });
  };
